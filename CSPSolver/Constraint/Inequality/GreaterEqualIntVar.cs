...
namespace CSPSolver.Constraint.Inequality
{
    public readonly struct GreaterEqualIntVar : IConstraint
    {
        ...
        {
            if (_var1.TryGetValue(state, out int val1)) return _var2.SetMin(state, val1 + 1) ? new IVariable[] { _var2 } : Enumerable.Empty<IVariable>();
            if (_var2.TryGetValue(state, out int val2)) return _var1.SetMax(state, val2 - 1) ? new IVariable[] { _var1 } : Enumerable.Empty<IVariable>();

            var changed = new List<IVariable>();

            if (_var1.SetMax(state, _var2.GetDomainMax(state) - 1)) changed.Add(_var1);
            if (_var2.SetMin(state, _var1.GetDomainMin(state) + 1)) changed.Add(_var2);
