...
{
    public readonly struct Ceil : IIntVar, ICompoundVariable
    {
        private readonly IRealVar _realVar;

        public Ceil(IRealVar realVar) => _realVar = realVar;

        public int Min => (int)Ceiling(_realVar.Min);

        public int Size => Max - Min + 1;

        public int Max => (int)Ceiling(_realVar.Max);

        public IEnumerable<IVariable> GetChildren() { yield return _realVar; }

        public int GetDomainMax(in IState state) => (int)Ceiling(_realVar.GetDomainMax(state));

        public int GetDomainMin(in IState state) => (int)Ceiling(_realVar.GetDomainMin(state));
        ...
        public bool SetMax(IState state, int max) => _realVar.SetMax(state, max);

        public bool SetMin(IState state, int min) => _realVar.SetMin(state, min - 1 + _realVar.Epsilon);
