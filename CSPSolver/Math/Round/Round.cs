...
namespace CSPSolver.Math.Round
{
    public readonly struct Round : IIntVar, ICompoundVariable
    {
        ...
        public int GetDomainMax(in IState state) => (int)System.Math.Round(_realVar.GetDomainMax(state));

        public int GetDomainMin(in IState state) => (int)System.Math.Round(_realVar.GetDomainMin(state));
