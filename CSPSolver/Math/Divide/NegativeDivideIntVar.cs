...
namespace CSPSolver.Math.Divide
{
    public readonly struct NegativeDivideIntVar : IIntVar, ICompoundVariable
    {
        ...
        public int GetDomainMax(in IState state) => _v1.GetDomainMin(state) / _v2.GetDomainMax(state);

        public int GetDomainMin(in IState state) => _v1.GetDomainMax(state) / _v2.GetDomainMin(state);
